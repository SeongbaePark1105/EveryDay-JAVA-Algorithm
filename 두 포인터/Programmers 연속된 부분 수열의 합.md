...
> 두 포인터로 해결할 수 있습니다.
>
> 조건에 맞게 최소 인덱스로 구할 수 있게 구현했고 끝 포인터가 찾는 숫자와 같을 경우 증가시켜줘서 `end`값이 배열의 길이와 같아 질 경우 `start`만 계속 증가시켜 무한 루프를 벗어날 수 있게 해주었습니다.
        ...
        while(start<len){
            ...
            if (sum == k){
                if (end - 1  - start < size){
                    endMin = end - 1 ;
                    startMin = start;
                    size = endMin - startMin;
                } else {
                    endMin = Math.min(end, endMin);
